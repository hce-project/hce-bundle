  ...
  then
    ...
    then
      ...
      exit 1
    	...
    	./postprocessor_task.py --config=../ini/postprocessor_task-rt.ini < $TOUTFILE > $TPOSTFILE
    
      if [ $? -ne 0 ] 
      then
        ./rtc-finalizer.py -c=../ini/rtc-finalizer.ini -rb $2 -rc $EXIT_CODE_RETRY < $TOUTFILE
      else
        ./rtc-finalizer.py -c=../ini/rtc-finalizer.ini -rb $2 -rc $EXIT_CODE_RETRY < $TPOSTFILE
      fi
