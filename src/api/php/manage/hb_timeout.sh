...
## $1 - input for check value
{
	...
	then
		...
		echo "Timeout value was inputted is incorrect."
...
if [ "$1" = "" ]; then
  COMMAND="NODE_HB_TIMEOUT_GET"
  HB_TIMEOUT=""
else
  COMMAND="NODE_HB_TIMEOUT_SET"
  HB_TIMEOUT="--hbtimeout=$1"
	checkAllowedValue "$1"
fi
...
## neither input parameters
{
    ...
    $BIN_DIR$MANAGER_COMMAND --command=$COMMAND --host=$CMD_HOSTS_CSV --port=$CMD_PORTS_CSV --handler=$CMD_HANDLER $HB_TIMEOUT --timeout=$SMALL_TIMEOUT
	else
