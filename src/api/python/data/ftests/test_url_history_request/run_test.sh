...
TEST_DIR=~/hce-node-bundle/api/python/data/ftests/test_url_history_request
...
HISTORY_FILE="$TMP_DIR/history.value.tmp"

HISTORY_URL_FILES=$TEST_DIR/url_history*.json
for f in $HISTORY_URL_FILES
do
	echo "Url history request"
  echo $f >> ../log/$0.log 2>&1
  ./dc-client.py --config=../ini/dc-client.ini --command=URL_HISTORY --file=$f > "$TMP_CONTENT_FILE" 2>&1
	cat "$TMP_CONTENT_FILE" >> ../log/$0.log 2>&1

	$BIN/json-field.py -f "itemsList:0:itemObject:0:logRows:0:Object" < "$TMP_CONTENT_FILE" > "$HISTORY_FILE"
	HISTORY=`cat "$HISTORY_FILE"`

	if [[ -n $HISTORY ]]
	then
		echo -ne "${GREEN}History is allowed${NC}"
		EXIT_CODE=0
	else
		echo -ne "${RED}History is not allowed${NC}"
		EXIT_CODE=1
	fi
	echo ""
done
